#!/usr/bin/env bash
# Generates the autodev-factory runtime prompt bundle from canonical autodev agent definitions.
#
# The `autodev-factory` runtime cannot delegate to plugin agents directly: a factory subagent is
# spawned through `ctx.agent(prompt)`, which has no `agent_type`, so it only ever sees the built-in
# agents, and the reviewer and worker instructions therefore have to travel *in the prompt*.
#
...
generated="$WORK_DIR/prompts.generated.mjs"
{
  ...
  echo "//"
  echo "// Each entry is the body of a canonical '.agent.md' from the consolidated autodev plugin,"
  echo "// together with the model its frontmatter declares. The autodev-factory runtime spawns subagents"
