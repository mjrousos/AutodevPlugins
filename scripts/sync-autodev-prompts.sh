#!/usr/bin/env bash
# Generates the autodev-factory prompt bundle from the canonical agent definitions.
#
# The `autodev-factory` factory cannot delegate to the plugin agents the way the `autodev-plan` and
...
OUTPUT_REL="plugins/autodev-factory/extensions/autodev-factory/prompts.generated.mjs"
...
# the part the factory replaces with code. All of them now ship in the merged autodev plugin.
SOURCE_FILES=(
  "plugins/autodev/agents/autodev-architecture-review.agent.md"
  "plugins/autodev/agents/autodev-security-review.agent.md"
  "plugins/autodev/agents/autodev-privacy-review.agent.md"
  "plugins/autodev/agents/autodev-tasking.agent.md"
  "plugins/autodev/agents/autodev-implementation.agent.md"
  "plugins/autodev/agents/autodev-code-review.agent.md"
  "plugins/autodev/agents/autodev-code-fix.agent.md"
  "plugins/autodev/agents/autodev-code-security-review.agent.md"
  "plugins/autodev/agents/autodev-code-privacy-review.agent.md"
)
...
PLAN_TEMPLATE_SOURCE="plugins/autodev/agents/autodev-plan.agent.md"
...
generated="$WORK_DIR/prompts.generated.mjs"
{
  ...
  echo "//"
  echo "// Each entry is the body of a canonical '.agent.md' from the merged autodev plugin,"
  echo "// together with the model its frontmatter declares. The autodev-factory factory spawns subagents"
...
# Windows jq writes CRLF even when invoked from Git Bash. Normalize the assembled module before
# either comparing or copying it so generate followed by --check is stable on every platform.
normalized="$WORK_DIR/prompts.normalized.mjs"
strip_cr "$generated" > "$normalized"
