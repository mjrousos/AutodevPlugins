...
#
# Usage:
#   plugins/autodev/install.sh              install for the current user (~/.copilot/extensions)
#   plugins/autodev/install.sh --project    install into this repository (.github/extensions)
#   plugins/autodev/install.sh --uninstall  remove a user-scope install
#
...
case "${1:-}" in
  ...
  *)
    ...
    echo "usage: plugins/autodev/install.sh [--user | --project | --uninstall]" >&2
    exit 2
    ;;
esac

SOURCE_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/extensions/autodev" >/dev/null 2>&1 && pwd)" || {
...
if [ "$MODE" = "project" ]; then
  ...
  TARGET_DIR="$repo_root/.github/extensions/autodev"
else
  TARGET_DIR="$copilot_home/extensions/autodev"
fi
...
echo "installed autodev extension -> $TARGET_DIR"
echo "run /extensions in Copilot CLI (or restart it), then invoke the 'autodev' factory."
