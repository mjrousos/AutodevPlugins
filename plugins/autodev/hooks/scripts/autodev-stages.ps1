...
.DESCRIPTION
    ...
    Enforcement state lives at '<COPILOT_HOME>/autodev/stages/<sessionId>.json', outside
...
function Get-StateDirectory {
    ...
    return (Join-Path (Join-Path $home_ 'autodev') 'stages')
}
...
function Test-EnforcementStateAbsent {
    ...
    try {
        ...
        $stateDir = [IO.Path]::Combine([IO.Path]::Combine($copilotHome, 'autodev'), 'stages')
