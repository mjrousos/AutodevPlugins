...
function Confirm-Directory {
    # Called only once a real autodev sub-agent has been identified, so directories
...
function Add-FeedbackEntry {
    ...
    try {
        ...
        if (-not (Test-Path -LiteralPath $Path)) {
            $header = @(
                '# autodev sub-agent feedback log',
                '',
                $sessionMarker,
                "Started: $((Get-Date).ToUniversalTime().ToString('u'))",
                '',
...
function Resolve-Agent {
    ...
    if ($AgentName -match '^\s*(?:autodev:)?autodev-(tasking|implementation|code-review|code-fix|code-security-review|code-privacy-review)\s*$') {
        return $Matches[1]
    }
    return $null
}
...
function Get-NextAction {
    ...
    switch ($Stage) {
        'idle' { return 'Invoke autodev:autodev-tasking to break the plan into milestones.' }
        'tasking' {
            if ([string]$State['taskingVerdict'] -eq 'BLOCKED') {
                return 'Re-invoke autodev:autodev-tasking with the context it said it was missing.'
            }
            return 'Invoke autodev:autodev-tasking to break the plan into milestones.'
        }
        'milestones' {
            ...
            if ([string]$State['implementVerdict'] -ne 'DONE') {
                if ([string]$State['implementVerdict'] -eq 'BLOCKED') {
                    return "Re-invoke autodev:autodev-implementation for milestone $m with the context it said it was missing."
                }
                return "Invoke autodev:autodev-implementation for milestone $m."
            }
            if ([string]$State['reviewVerdict'] -eq 'ISSUES') {
                $remaining = $script:MaxReviewAttempts - [int]$State['reviewAttempts']
                return "Invoke autodev:autodev-code-fix with the findings above verbatim, then re-invoke autodev:autodev-code-review for milestone $m. $remaining round(s) remain before the findings are recorded as unresolved and the run moves on."
            }
            return "Invoke autodev:autodev-code-review for milestone $m."
        }
        'user-review' {
            ...
            return 'The user has the code. When they tell you to proceed, invoke autodev:autodev-code-security-review. If they report issues, invoke autodev:autodev-code-fix and hand the result back to them again.'
        }
        'security' {
            if ([string]$State['securityVerdict'] -eq 'ISSUES') {
                $remaining = $script:MaxReviewAttempts - [int]$State['securityAttempts']
                return "Invoke autodev:autodev-code-fix with the findings above verbatim, then re-invoke autodev:autodev-code-security-review. $remaining round(s) remain before escalation."
            }
            return 'Invoke autodev:autodev-code-security-review.'
        }
        'privacy' {
            if ([string]$State['privacyVerdict'] -eq 'ISSUES') {
                $remaining = $script:MaxReviewAttempts - [int]$State['privacyAttempts']
                return "Invoke autodev:autodev-code-fix with the findings above verbatim, then re-invoke autodev:autodev-code-privacy-review. $remaining round(s) remain before escalation."
            }
...
try {
    ...
    switch ($EventName) {
        ...
        'agentStop' {
            ...
            $statusLine = Get-StatusLine -State $state
            $reason = "You stopped while the autodev run is still in progress. " +
            "Run status: $statusLine. " +
        ...
        'preToolUse' {
            ...
            if (-not (Test-Path -LiteralPath $statePath) -and
                ...
                if ($toolName -eq 'task') {
                    ...
                    if ($null -ne $target -and $target -ne 'tasking') {
                        ...
                        "autodev run has not started and no milestones exist yet. " +
                        "Next required action: Invoke autodev:autodev-tasking to break " +
            ...
            if ($toolName -eq 'task') {
                ...
                if ($stage -eq 'escalated') {
                    $reason = "The autodev run is out of budget: $(Get-EscalationReason -State $state). " +
                ...
                if ($stage -eq 'idle') {
                    ...
                    if ($target -ne 'tasking') {
                        $denyReason = "the autodev run has not started and no milestones exist yet, so tasking has to define them before any other stage runs"
                    }
                }
            ...
            $reason = "The autodev run is in an autonomous phase ($stage), which proceeds " +
