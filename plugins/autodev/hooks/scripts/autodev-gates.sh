...
STATE_DIR="$COPILOT_HOME_DIR/autodev/gates"
...
add_audit_row() {
  ...
  if [ ! -f "$AUDIT_PATH" ]; then
    {
      printf '# autodev review gate audit\n\n'
      printf '%s\n' "$session_marker"
...
add_feedback_entry() {
  ...
  if [ ! -f "$FEEDBACK_PATH" ]; then
    {
      printf '# autodev reviewer feedback log\n\n'
      printf '%s\n' "$session_marker"
      printf 'Started: %s\n\n' "$(date -u '+%Y-%m-%d %H:%M:%S UTC')"
...
# agentName arrives namespaced, e.g. "autodev:autodev-privacy-review". Trailing whitespace
...
$NEXT_ACTION"
    ...
    send_otel_span "$(jq -cn \
      ...
      '{spanName: $span, sessionId: $sid, agentName: $agentName, agentId: $agentId,
        plugin: "autodev", unitKey: "autodev.gate", unitValue: $unitValue,
