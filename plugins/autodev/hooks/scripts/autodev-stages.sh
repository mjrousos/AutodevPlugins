#!/usr/bin/env bash
# Stage tracker for the merged autodev plugin (Linux / macOS).
#
...
STATE_DIR="$COPILOT_HOME_DIR/autodev/stages"
    ...
    if [ ! -f "$STATE_PATH" ] && [ ! -f "$MIRROR_PATH" ]; then
      ...
      if [ "$TOOL_NAME" = "task" ]; then
        ...
        if [ -n "$TARGET" ] && [ "$TARGET" != "tasking" ]; then
          REASON="Out of order: invoking $TARGET is not the next step because the autodev-implement run has not started and no milestones exist yet. Next required action: Invoke autodev:autodev-tasking to break the plan into milestones."
          jq -cn --arg r "$REASON" \
