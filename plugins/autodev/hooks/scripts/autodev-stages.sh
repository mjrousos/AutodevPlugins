#!/usr/bin/env bash
# Stage tracker for the autodev plugin (Linux / macOS).
#
...
# Called only once a real autodev sub-agent has been identified, so directories appear
...
add_feedback_entry() {
  ...
  if [ ! -f "$FEEDBACK_PATH" ]; then
    {
      printf '# autodev sub-agent feedback log\n\n'
      printf '%s\n' "$session_marker"
      printf 'Started: %s\n\n' "$(date -u '+%Y-%m-%d %H:%M:%S UTC')"
...
# agentName arrives namespaced, e.g. "autodev:autodev-code-review". The match is
...
# 'code-security-review', and what keeps this stage tracker from capturing the planning
# gate agents.
resolve_agent() {
  local name
  name="$(printf '%s' "$1" | sed 's/^[[:space:]]*//; s/[[:space:]]*$//')"
  case "$name" in
    autodev-tasking|autodev:autodev-tasking)
      printf 'tasking' ;;
    autodev-implementation|autodev:autodev-implementation)
      printf 'implementation' ;;
    autodev-code-review|autodev:autodev-code-review)
      printf 'code-review' ;;
    autodev-code-fix|autodev:autodev-code-fix)
      printf 'code-fix' ;;
    autodev-code-security-review|autodev:autodev-code-security-review)
      printf 'code-security-review' ;;
    autodev-code-privacy-review|autodev:autodev-code-privacy-review)
...
get_next_action() {
  ...
  case "$stage" in
    idle)
      printf 'Invoke autodev:autodev-tasking to break the plan into milestones.'
      ;;
    tasking)
      if [ "$(state_str "$state" 'taskingVerdict')" = "BLOCKED" ]; then
        printf 'Re-invoke autodev:autodev-tasking with the context it said it was missing.'
      else
        printf 'Invoke autodev:autodev-tasking to break the plan into milestones.'
      fi
      ;;
    milestones)
      ...
      if [ "$(state_str "$state" 'implementVerdict')" != "DONE" ]; then
        if [ "$(state_str "$state" 'implementVerdict')" = "BLOCKED" ]; then
          printf 'Re-invoke autodev:autodev-implementation for milestone %s with the context it said it was missing.' "$m"
        else
          printf 'Invoke autodev:autodev-implementation for milestone %s.' "$m"
        fi
      elif [ "$(state_str "$state" 'reviewVerdict')" = "ISSUES" ]; then
        remaining=$(( MAX_REVIEW_ATTEMPTS - $(state_num "$state" 'reviewAttempts') ))
        printf 'Invoke autodev:autodev-code-fix with the findings above verbatim, then re-invoke autodev:autodev-code-review for milestone %s. %s round(s) remain before the findings are recorded as unresolved and the run moves on.' "$m" "$remaining"
      else
        printf 'Invoke autodev:autodev-code-review for milestone %s.' "$m"
      fi
      ;;
    user-review)
      if [ "$(state_num "$state" 'userReviewReached')" -eq 0 ] 2>/dev/null; then
        ...
        printf 'The user has the code. When they tell you to proceed, invoke autodev:autodev-code-security-review. If they report issues, invoke autodev:autodev-code-fix and hand the result back to them again.'
      fi
      ;;
    security)
      if [ "$(state_str "$state" 'securityVerdict')" = "ISSUES" ]; then
        remaining=$(( MAX_REVIEW_ATTEMPTS - $(state_num "$state" 'securityAttempts') ))
        printf 'Invoke autodev:autodev-code-fix with the findings above verbatim, then re-invoke autodev:autodev-code-security-review. %s round(s) remain before escalation.' "$remaining"
      else
        printf 'Invoke autodev:autodev-code-security-review.'
      fi
      ;;
    privacy)
      if [ "$(state_str "$state" 'privacyVerdict')" = "ISSUES" ]; then
        remaining=$(( MAX_REVIEW_ATTEMPTS - $(state_num "$state" 'privacyAttempts') ))
        printf 'Invoke autodev:autodev-code-fix with the findings above verbatim, then re-invoke autodev:autodev-code-privacy-review. %s round(s) remain before escalation.' "$remaining"
      else
...
#
# preToolUse is deliberately excluded. Unlike autodev-gates.sh, its no-state branch
...
Feedback log: $FEEDBACK_PATH"
    ...
    send_otel_span "$(jq -cn \
      ...
      '{spanName: $span, sessionId: $sid, agentName: $agentName, agentId: $agentId,
        plugin: "autodev", unitKey: "autodev.stage", unitValue: $unitValue,
...
$FOOTER" '{modifiedResponse: $r}' 2>/dev/null || emit_empty
  ...
  agentStop)
    ...
    STATUS_LINE="$(get_status_line "$STATE")"
    REASON="You stopped while the autodev run is still in progress. Run status: $STATUS_LINE. Do not end your turn and do not ask the user anything. Continue the workflow now. Next required action: $(get_next_action "$STATE" "$STAGE")"
    ...
    if [ ! -f "$STATE_PATH" ] && [ ! -f "$MIRROR_PATH" ]; then
      ...
      if [ "$TOOL_NAME" = "task" ]; then
        ...
        if [ -n "$TARGET" ] && [ "$TARGET" != "tasking" ]; then
          REASON="Out of order: invoking $TARGET is not the next step because the autodev run has not started and no milestones exist yet. Next required action: Invoke autodev:autodev-tasking to break the plan into milestones."
          jq -cn --arg r "$REASON" \
    ...
    if [ "$TOOL_NAME" = "task" ]; then
      ...
      if [ "$STAGE" = "idle" ]; then
        ...
        if [ "$TARGET" != "tasking" ]; then
          DENY_REASON="the autodev run has not started and no milestones exist yet, so tasking has to define them before any other stage runs"
        fi
