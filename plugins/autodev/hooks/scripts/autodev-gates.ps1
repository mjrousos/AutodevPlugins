...
function Resolve-Gate {
    ...
    # agentName arrives namespaced, e.g. "autodev:autodev-privacy-review". The match is anchored
    # at both ends and pinned to this plugin's namespace, because a suffix match would also
    # capture another installed plugin's "other:autodev-privacy-review" and let it mutate this
...
function Test-EnforcementStateAbsent {
    ...
    try {
        ...
        $stateDir = [IO.Path]::Combine([IO.Path]::Combine($copilotHome, 'autodev-plan'), 'gates')
