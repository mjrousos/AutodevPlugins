/**
 * Unit tests for the autodev factory's pure logic.
 *
 ...
 *
 * Run: node --test plugins/autodev/tests/
 */
...
const {
    ...
    sanitizeKey,
    truncate,
} = await import("../extensions/autodev/extension.mjs");
...
test("the factory is registered with every phase it reports", () => {
    assert.equal(autodev.meta.name, "autodev");
    assert.equal(autodev.meta.phases.length, 14);
    for (const phase of autodev.meta.phases) {
...
test("the description documents the argument shape, which is all the model gets", () => {
    ...
    for (const argument of ["request", "repoRoot", "planPath", "todosPath", "startAt", "clarifyRounds"]) {
        assert.match(autodev.meta.description, new RegExp(argument), `description documents ${argument}`);
    }
});

test("no declared limits — every loop bounds itself", () => {
    assert.equal(autodev.meta.limits, undefined);
