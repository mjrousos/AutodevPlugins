...
t_hooks_dispatch() {
  local bad
  # Each entry must invoke one of the two router scripts and pass its own event name.
  bad="$(hooks_q '[.hooks | to_entries[] | .key as $k | .value[]
    | select((.bash | test("autodev-(gates|stages)\\.sh\" " + $k + "$") | not)
          or (.powershell | test("autodev-(gates|stages)\\.ps1\" " + $k + "$") | not)
