#!/usr/bin/env bash
# Tests for the autodev gate tracker (autodev-gates.sh).
#
...
if [ "$SHARD" -lt 0 ] && [ "$SEQUENTIAL" -eq 0 ]; then
  ...
  echo
  echo "autodev gate tracker tests (bash), $WORKERS workers"
...
state_path()    { printf '%s/autodev/gates/%s.json' "$COPILOT_HOME" "$1"; }
...
start_gate() { # sid gate
  hook subagentStart "$(jq -cn --arg s "$1" --arg g "$2" --arg c "$(session_cwd "$1")" \
    '{sessionId:$s, cwd:$c, agentName:("autodev:autodev-" + $g + "-review")}')" > /dev/null
}

stop_gate() { # sid gate response
  hook subagentStop "$(jq -cn --arg s "$1" --arg g "$2" --arg r "$3" --arg c "$(session_cwd "$1")" \
    '{sessionId:$s, cwd:$c, agentName:("autodev:autodev-" + $g + "-review"), response:$r}')"
}
...
reviewer_task() { # sid gate
  ...
  local cwd args
  cwd="$(session_cwd "$1")"
  args="$(jq -cn --arg a "autodev:autodev-$2-review" '{agent_type:$a, prompt:"review"}')"
...
section "autodev gate tracker tests (bash)"
...
for AGENT in explore general-purpose security-review code-review task \
  other-plugin:autodev-security-review other:autodev-architecture-review; do
  # shellcheck disable=SC2317
  t_non_reviewer() {
    local out
...
t_semantic_corruption_uses_mirror() {
  ...
  for spec in \
    ...
    assert_match 'autodev:autodev-security-review' \
...
t_path_traversal() {
  ...
  hook subagentStart "$(jq -cn --arg c "$safe" \
    '{sessionId:"../../evil", cwd:$c, agentName:"autodev:autodev-security-review"}')" > /dev/null
...
t_block_while_outstanding() {
  ...
  assert_match 'autodev:autodev-architecture-review' "$(printf '%s' "$out" | jq -r '.reason // ""')"
}
...
t_names_next_gate() {
  ...
  sid="$(new_session_id)"
  round "$sid" architecture PASS > /dev/null
  assert_match 'autodev:autodev-security-review' "$(agent_stop "$sid" | jq -r '.reason // ""')"
}
...
t_regate_invalidates_downstream() {
  ...
  assert_match 'autodev:autodev-security-review' "$(printf '%s' "$out" | jq -r '.reason // ""')"
}
...
t_agentstop_when_state_dir_cannot_be_recreated() {
  ...
  sid="$(new_session_id)"
  round "$sid" architecture PASS > /dev/null
  state_root="$COPILOT_HOME/autodev"
  rm -rf "$state_root"
...
t_other_session_mirror_not_recovered() {
  ...
  hook subagentStart "$(jq -cn --arg s "$first" --arg c "$shared" \
    '{sessionId:$s, cwd:$c, agentName:"autodev:autodev-architecture-review"}')" > /dev/null
  hook subagentStop "$(jq -cn --arg s "$first" --arg c "$shared" \
    '{sessionId:$s, cwd:$c, agentName:"autodev:autodev-architecture-review",
...
t_two_sessions_independent() {
  ...
  assert_equal 'deny' "$(hook preToolUse "$(jq -cn --arg s "$a" --arg c "$shared" \
    '{sessionId:$s, cwd:$c, toolName:"task",
      toolArgs:"{\"agent_type\":\"autodev:autodev-architecture-review\"}"}')" | jq -r '.permissionDecision // ""')" \
    'the session at its cap must be refused' || return 1
  assert_equal '{}' "$(hook preToolUse "$(jq -cn --arg s "$b" --arg c "$shared" \
    '{sessionId:$s, cwd:$c, toolName:"task",
      toolArgs:"{\"agent_type\":\"autodev:autodev-architecture-review\"}"}')")" \
...
t_new_session_preserves_logs() {
  ...
  for gate in architecture security privacy; do
    ...
    hook subagentStop "$(jq -cn --arg s "$first" --arg c "$shared" --arg g "$gate" \
      '{sessionId:$s, cwd:$c, agentName:("autodev:autodev-" + $g + "-review"),
        response:"ok\n\nAUTODEV-VERDICT: PASS"}')" > /dev/null
  done
...
t_hooks_dispatch() {
  ...
  bad="$(hooks_q '[.hooks | to_entries[] | .key as $k | .value[]
    | select((.bash | test("autodev-router\\.sh\" " + $k + "$") | not)
          or (.powershell | test("autodev-router\\.ps1\" " + $k + "$") | not)
...
t_hooks_scripts_exist() {
  local rel
  for rel in hooks/scripts/autodev-router.ps1 hooks/scripts/autodev-router.sh; do
...
t_otel_correlation_attributes() {
  ...
  assert_equal 'autodev:autodev-security-review' \
...
t_otel_missed_start_still_counts_the_invocation() {
  ...
  cwd="$(session_cwd "$sid")"
  sink="$(telemetry_dir "$sid")/spans.jsonl"
  # Deliberately no start_gate.
  COPILOT_OTEL_ENABLED=true AUTODEV_OTEL_DEBUG_FILE="$sink" \
    hook subagentStop "$(jq -cn --arg s "$sid" --arg c "$cwd" \
      '{sessionId:$s, cwd:$c, agentName:"autodev:autodev-architecture-review",
        agentId:"a1", response:"x\n\nAUTODEV-VERDICT: ISSUES"}')" >/dev/null
  assert_equal 1 "$(span_attr "$sink" 'autodev.attempt')" || return 1
...
t_otel_span_marks_the_verdict_instant() {
  ...
  COPILOT_OTEL_ENABLED=true AUTODEV_OTEL_DEBUG_FILE="$sink" \
    hook subagentStop "$(jq -cn --arg s "$sid" --arg c "$cwd" \
      '{sessionId:$s, cwd:$c, agentName:"autodev:autodev-architecture-review", agentId:"a1",
