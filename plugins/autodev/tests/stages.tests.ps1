<#
.SYNOPSIS
    Tests for the autodev stage tracker (autodev-stages.ps1).
...
if (-not $script:IsWorker -and -not $Sequential) {
    ...
    Write-Host ''
    Write-Host "autodev stage tracker tests (PowerShell), $Workers workers" -ForegroundColor Cyan
...
function Get-StatePath {
    ...
    return (Join-Path (Join-Path $script:Root 'autodev\stages') "$SessionId.json")
}
...
function Start-Agent {
    ...
    Invoke-Hook 'subagentStart' @{ sessionId = $SessionId; agentName = "autodev:autodev-$Agent" } | Out-Null
}

function Stop-Agent {
    param([string]$SessionId, [string]$Agent, [string]$Response)
    return Invoke-Hook 'subagentStop' @{ sessionId = $SessionId; agentName = "autodev:autodev-$Agent"; response = $Response }
}
...
function Invoke-TaskCheck {
    ...
    param([string]$SessionId, [string]$Agent)
    $taskArgs = @{ agent_type = "autodev:autodev-$Agent" } | ConvertTo-Json -Compress
...
Write-Host ''
Write-Host 'autodev stage tracker tests (PowerShell)' -ForegroundColor Cyan
...
Test-Case 'a stray mirror in the shared state directory does not mask a live run' {
    ...
    $stray = Join-Path (Join-Path $script:Root 'autodev\stages') 'implement-status.json'
...
Test-Case 'a workspace path needing JSON unescaping still resolves to the same answer' {
    ...
    Invoke-Hook 'subagentStart' @{ sessionId = $sid; cwd = $cwd; agentName = 'autodev:autodev-tasking' } | Out-Null
...
foreach ($agent in @('explore', 'general-purpose', 'security-review', 'code-review', 'task', 'autodev:autodev-implement')) {
    $a = $agent
...
foreach ($agent in @('autodev:autodev-architecture-review', 'autodev:autodev-security-review', 'autodev:autodev-privacy-review')) {
    $a = $agent
...
Test-Case 'a hostile session id cannot escape the state directory' {
    $sid = '../../escaped'
    Invoke-Hook 'subagentStart' @{ sessionId = $sid; cwd = (Get-SessionCwd 'hostile'); agentName = 'autodev:autodev-tasking' } | Out-Null
...
Test-Case 'a todo list with no milestone headings is recorded as BLOCKED, not DONE' {
    ...
    Assert-Match 'Re-invoke autodev:autodev-tasking' $footer
...
Test-Case 'a tasking DONE is trusted when there is no workspace to check it against' {
    ...
    $sid = New-SessionId
    Invoke-Hook 'subagentStart' @{ sessionId = $sid; cwd = ''; agentName = 'autodev:autodev-tasking' } | Out-Null
    $out = Invoke-Hook 'subagentStop' @{ sessionId = $sid; cwd = ''; agentName = 'autodev:autodev-tasking'; response = "body`n`nAUTODEV-VERDICT: DONE" }
...
Test-Case 'the block reason names the exact next sub-agent' {
    ...
    Set-StageState -SessionId $sid -Stage 'milestones'
    $out = Invoke-Hook 'agentStop' @{ sessionId = $sid }
    Assert-Match 'autodev:autodev-code-review for milestone 1' $out
}
...
Test-Case 'a task call to an agent from the plan workflow is never refused' {
    $sid = New-SessionId
    Set-StageState -SessionId $sid -Stage 'milestones'
    $taskArgs = @{ agent_type = 'autodev:autodev-security-review' } | ConvertTo-Json -Compress
    Assert-Equal '{}' (Invoke-Hook 'preToolUse' @{ sessionId = $sid; toolName = 'task'; toolArgs = $taskArgs })
}
...
function Invoke-TelemetryRound {
    ...
    $agentName = "autodev:autodev-$Agent"
...
Test-Case 'telemetry is off by default and writes nothing' {
    ...
    Invoke-HookWithEnv 'subagentStart' @{ sessionId = $sid; agentName = 'autodev:autodev-tasking' } @{ AUTODEV_OTEL_DEBUG_FILE = $sink } | Out-Null
    $out = Invoke-HookWithEnv 'subagentStop' @{
        sessionId = $sid
        agentName = 'autodev:autodev-tasking'
...
Test-Case 'AUTODEV_OTEL_ENABLED alone turns telemetry on' {
    ...
    Invoke-HookWithEnv 'subagentStart' @{ sessionId = $sid; agentName = 'autodev:autodev-tasking' } $vars | Out-Null
...
Test-Case 'an enabled round emits exactly one well-formed span document' {
    ...
    Assert-Equal 'autodev' (Get-SpanAttribute $result.Spans[0] 'autodev.plugin')
    Assert-Equal 'tasking' (Get-SpanAttribute $result.Spans[0] 'autodev.stage')
}

Test-Case 'autodev.issues counts the reported findings' {
    <#
        The attribute is a COUNT of findings, not a flag: "did this stage come back dirty" is
        already answered by autodev.verdict = ISSUES. Counted from the '### [severity] title'
        heading every reviewer agent is required to emit.
    #>
    $sid = New-SessionId
    Set-TodoList -SessionId $sid -Milestones 1
    $sink = Join-Path (Get-TelemetryDir $sid) 'spans.jsonl'
    $vars = @{ AUTODEV_OTEL_ENABLED = 'true'; AUTODEV_OTEL_DEBUG_FILE = $sink }
    $agentName = 'autodev:autodev-code-review'
    $response = @(
        '### [major] First problem',
    ...
    Invoke-HookWithEnv 'subagentStop' @{
        sessionId = $sid; agentName = $agentName; agentId = 'a1'; response = $response
    } $vars | Out-Null
    $doc = (Get-Content -LiteralPath $sink | Select-Object -First 1) | ConvertFrom-Json
...
Test-Case 'a stop without its start still counts as an invocation' {
    ...
    Invoke-HookWithEnv 'subagentStop' @{
        sessionId = $sid; agentName = 'autodev:autodev-code-review'; agentId = 'a1'
        response  = "x`n`nAUTODEV-VERDICT: ISSUES"
