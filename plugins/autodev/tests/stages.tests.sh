...
# Both trackers run from one plugin and both watch subagentStart. The stage tracker must not
# capture planning reviewers, or a planning run would spend an implementation budget.
PLAN_AGENT=""
...
for agent in autodev:autodev-architecture-review autodev:autodev-security-review autodev:autodev-privacy-review; do
  PLAN_AGENT="$agent"
  run_test "planning reviewer '$agent' is not captured" t_plan_agent_ignored
done
...
t_otel_autodev_enabled_alone() {
  ...
  # hook sees them. The full enablement matrix lives in the gate suite, against the same
  # canonical emitter.
  local sid sink
  sid="$(new_session_id)"
  set_todo_list "$sid" 1
