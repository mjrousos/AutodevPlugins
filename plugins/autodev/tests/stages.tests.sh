#!/usr/bin/env bash
# Tests for the autodev stage tracker (autodev-stages.sh).
#
...
if [ "$SHARD" -lt 0 ] && [ "$SEQUENTIAL" -eq 0 ]; then
  ...
  echo
  echo "autodev stage tracker tests (bash), $WORKERS workers"
...
state_path() { printf '%s/autodev/stages/%s.json' "$COPILOT_HOME" "$1"; }
...
start_agent() { # sid agent
  hook subagentStart "$(jq -cn --arg s "$1" --arg a "$2" --arg c "$(session_cwd "$1")" \
    '{sessionId:$s, cwd:$c, agentName:("autodev:autodev-" + $a)}')" > /dev/null
}

stop_agent() { # sid agent response
  hook subagentStop "$(jq -cn --arg s "$1" --arg a "$2" --arg r "$3" --arg c "$(session_cwd "$1")" \
    '{sessionId:$s, cwd:$c, agentName:("autodev:autodev-" + $a), response:$r}')"
}
...
agent_task_check() { # sid agent
  task_check "$1" "autodev:autodev-$2"
}
...
echo
echo "autodev stage tracker tests (bash)"
...
for agent in explore general-purpose security-review code-review task autodev:autodev-implement; do
  UNTRACKED_AGENT="$agent"
  run_test "untracked sub-agent '$agent' is ignored" t_untracked_agent
done

# Both workflows share the autodev namespace and both drive subagentStart. The stage tracker must
# never capture the gate reviewers, or a planning run would spend an implementation budget.
PLAN_AGENT=""
...
for agent in autodev:autodev-architecture-review autodev:autodev-security-review autodev:autodev-privacy-review; do
  PLAN_AGENT="$agent"
  run_test "autodev-plan gate reviewer '$agent' is not captured" t_plan_agent_ignored
done
...
t_hostile_session_id() {
  ...
  hook subagentStart "$(jq -cn --arg c "$cwd" \
    '{sessionId:"../../escaped", cwd:$c, agentName:"autodev:autodev-tasking"}')" > /dev/null
...
t_todo_no_headings() {
  ...
  assert_match 'Re-invoke autodev:autodev-tasking' "$footer" || return 1
...
t_no_workspace_tasking_trusted() {
  ...
  hook subagentStart "$(jq -cn --arg s "$sid" \
    '{sessionId:$s, cwd:"", agentName:"autodev:autodev-tasking"}')" > /dev/null
  footer="$(hook subagentStop "$(jq -cn --arg s "$sid" --arg r "body

AUTODEV-VERDICT: DONE" '{sessionId:$s, cwd:"", agentName:"autodev:autodev-tasking", response:$r}')" | jq -r '.modifiedResponse // ""')"
...
t_block_names_next_agent() {
  ...
  sid="$(new_session_id)"
  seed_stage "$sid" 'milestones'
  assert_match 'autodev:autodev-code-review for milestone 1' "$(agent_stop "$sid")"
}
...
t_other_plugin_task_allowed() {
  ...
  sid="$(new_session_id)"
  seed_stage "$sid" 'milestones'
  assert_equal '{}' "$(task_check "$sid" 'autodev:autodev-security-review')"
}
run_test 'a task call to a plan-workflow gate agent is never refused' t_other_plugin_task_allowed
...
telemetry_round() { # sid agent verdict [agentId]
  ...
  COPILOT_OTEL_ENABLED=true AUTODEV_OTEL_DEBUG_FILE="$TELEMETRY_SINK" \
    hook subagentStart "$(jq -cn --arg s "$sid" --arg a "$agent" --arg c "$cwd" \
      '{sessionId:$s, cwd:$c, agentName:("autodev:autodev-" + $a)}')" >/dev/null
  TELEMETRY_OUTPUT="$(COPILOT_OTEL_ENABLED=true AUTODEV_OTEL_DEBUG_FILE="$TELEMETRY_SINK" \
    hook subagentStop "$(jq -cn --arg s "$sid" --arg a "$agent" --arg i "$agent_id" --arg c "$cwd" --arg v "$verdict" \
      '{sessionId:$s, cwd:$c, agentName:("autodev:autodev-" + $a),
...
t_otel_disabled_by_default() {
  ...
  AUTODEV_OTEL_DEBUG_FILE="$sink" start_agent "$sid" tasking
  footer="$(AUTODEV_OTEL_DEBUG_FILE="$sink" stop_agent "$sid" tasking "x
...
t_otel_well_formed_span() {
  ...
  assert_equal 'autodev.stage tasking' "$(span_field "$TELEMETRY_SINK" name)" || return 1
  assert_equal autodev "$(span_attr "$TELEMETRY_SINK" 'autodev.plugin')" || return 1
  assert_equal tasking "$(span_attr "$TELEMETRY_SINK" 'autodev.stage')"
}
...
t_otel_review_issues() {
  ...
  sid="$(new_session_id)"
  set_todo_list "$sid" 1
  telemetry_round "$sid" code-review ISSUES
  ...
  assert_equal 0 "$(span_attr "$TELEMETRY_SINK" 'autodev.blocked')"
}
run_test 'a review ISSUES verdict is counted as an issue' t_otel_review_issues

t_otel_review_pass() {
  local sid
  sid="$(new_session_id)"
  set_todo_list "$sid" 1
  telemetry_round "$sid" code-security-review PASS
  assert_equal PASS "$(span_attr "$TELEMETRY_SINK" 'autodev.verdict')" || return 1
  assert_equal 0 "$(span_attr "$TELEMETRY_SINK" 'autodev.issues')" || return 1
  assert_equal 0 "$(span_attr "$TELEMETRY_SINK" 'autodev.blocked')"
}
run_test 'a review PASS verdict is not counted as an issue' t_otel_review_pass

t_otel_worker_blocked() {
  # The two vocabularies must stay separate. A blocked implementation worker is an operational
  # stall, not a review finding, so folding it into autodev.issues would inflate the ISSUES
  # count the whole feature exists to report.
  local sid
  sid="$(new_session_id)"
  set_todo_list "$sid" 1
  telemetry_round "$sid" implementation BLOCKED
  assert_equal BLOCKED "$(span_attr "$TELEMETRY_SINK" 'autodev.verdict')" || return 1
  assert_equal 0 "$(span_attr "$TELEMETRY_SINK" 'autodev.issues')" || return 1
  assert_equal 1 "$(span_attr "$TELEMETRY_SINK" 'autodev.blocked')"
}
run_test 'a BLOCKED worker counts as blocked, not as a review issue' t_otel_worker_blocked

t_otel_worker_done() {
  local sid
  sid="$(new_session_id)"
  set_todo_list "$sid" 1
  telemetry_round "$sid" implementation DONE
  assert_equal DONE "$(span_attr "$TELEMETRY_SINK" 'autodev.verdict')" || return 1
  assert_equal 0 "$(span_attr "$TELEMETRY_SINK" 'autodev.issues')" || return 1
  assert_equal 0 "$(span_attr "$TELEMETRY_SINK" 'autodev.blocked')"
}
run_test 'a DONE worker counts as neither' t_otel_worker_done
...
t_otel_raw_session_id() {
  ...
  COPILOT_OTEL_ENABLED=true AUTODEV_OTEL_DEBUG_FILE="$sink" \
    hook subagentStart "$(jq -cn --arg s "$raw" --arg c "$dir" \
      '{sessionId:$s, cwd:$c, agentName:"autodev:autodev-code-review"}')" >/dev/null
  COPILOT_OTEL_ENABLED=true AUTODEV_OTEL_DEBUG_FILE="$sink" \
    hook subagentStop "$(jq -cn --arg s "$raw" --arg c "$dir" \
      '{sessionId:$s, cwd:$c, agentName:"autodev:autodev-code-review",
...
t_otel_empty_agent_id_does_not_shift_fields() {
  ...
  assert_equal autodev "$(span_attr "$sink" 'autodev.plugin')" || return 1
  assert_equal code-review "$(span_attr "$sink" 'autodev.stage')"
}
...
t_otel_hostile_child_cannot_break_the_hook() {
  ...
  # into this process -- would destroy the tracker footer.
  local sid cwd sandbox out code
  sandbox="$COPILOT_HOME/otel-hostile-$$-$RANDOM"
  mkdir -p "$sandbox"
  cp "$STAGE_SCRIPT" "$sandbox/autodev-stages.sh"
...
HOSTILE
  ...
  out="$(printf '%s\n' "$(jq -cn --arg s "$sid" --arg c "$cwd" \
    ...
    COPILOT_OTEL_ENABLED=true bash "$sandbox/autodev-stages.sh" subagentStop)"
  code=$?
  ...
  assert_match 'stage tracker' "$(printf '%s' "$out" | jq -r '.modifiedResponse // ""')" \
...
t_otel_unreachable_collector() {
  ...
  assert_match 'stage tracker' "$footer" 'the footer must survive a failed export' || return 1
