<#
.SYNOPSIS
    Tests for the autodev gate tracker (autodev-gates.ps1).
...
if (-not $script:IsWorker -and -not $Sequential) {
    ...
    Write-Host ''
    Write-Host "autodev gate tracker tests (PowerShell), $Workers workers" -ForegroundColor Cyan
...
function Get-StatePath {
    ...
    return (Join-Path (Join-Path $script:Root 'autodev\gates') "$SessionId.json")
}
...
function Start-Gate {
    ...
    Invoke-Hook 'subagentStart' @{ sessionId = $SessionId; agentName = "autodev:autodev-$Gate-review" } | Out-Null
}

function Stop-Gate {
    param([string]$SessionId, [string]$Gate, [string]$Response)
    return Invoke-Hook 'subagentStop' @{ sessionId = $SessionId; agentName = "autodev:autodev-$Gate-review"; response = $Response }
}
...
Write-Host ''
Write-Host 'autodev gate tracker tests (PowerShell)' -ForegroundColor Cyan
...
foreach ($agent in @(
        ...
        'code-review',
        'task',
        'other-plugin:autodev-security-review',
        'other:autodev-architecture-review'
    )) {
    $a = $agent
...
Test-Case 'a state file with no owner is never adopted' {
    ...
    $args_ = '{"agent_type":"autodev:autodev-architecture-review"}'
...
Test-Case 'semantically corrupt authoritative state falls back to the valid mirror' {
    ...
    foreach ($case in $cases) {
        ...
        Assert-Match 'autodev:autodev-security-review' $stop.reason
...
Test-Case 'a hostile session id cannot escape the state directory' {
    $sid = '../../evil'
    Invoke-Hook 'subagentStart' @{ sessionId = $sid; agentName = 'autodev:autodev-security-review'; cwd = (Get-SessionCwd 'hostile-id-cwd') } | Out-Null
...
Test-Case 'agentStop is blocked while a gate is outstanding' {
    ...
    Assert-Match 'autodev:autodev-architecture-review' $parsed.reason
}
...
Test-Case 'invoking a reviewer is permitted while the gate still has attempts left' {
    $sid = New-SessionId
    Invoke-Round -SessionId $sid -Gate 'architecture' -Verdict 'ISSUES' | Out-Null
    $args_ = '{"agent_type":"autodev:autodev-architecture-review","prompt":"review"}'
    Assert-Equal '{}' (Invoke-Hook 'preToolUse' @{ sessionId = $sid; toolName = 'task'; toolArgs = $args_ })
}
...
Test-Case 'agentStop names the next gate once one passes' {
    ...
    Invoke-Round -SessionId $sid -Gate 'architecture' -Verdict 'PASS' | Out-Null
    $parsed = Invoke-Hook 'agentStop' @{ sessionId = $sid; stopReason = 'end_turn' } | ConvertFrom-Json
    Assert-Match 'autodev:autodev-security-review' $parsed.reason
}
...
function Invoke-ReviewerTask {
    param([string]$SessionId, [string]$Gate)
    $args_ = "{`"agent_type`":`"autodev:autodev-$Gate-review`",`"prompt`":`"review`"}"
...
Test-Case 'losing authoritative state between reviewers recovers from the mirror' {
    # Regression: deleting COPILOT_HOME/autodev mid-session made the next reviewer look like
    ...
    Remove-Item -LiteralPath (Join-Path $script:Root 'autodev') -Recurse -Force
...
Test-Case 'agentStop persists its counter to the mirror when external state cannot be recreated' {
    $sid = New-SessionId
    Invoke-Round -SessionId $sid -Gate 'architecture' -Verdict 'PASS' | Out-Null
    $stateRoot = Join-Path $script:Root 'autodev'
...
Test-Case 'a mirror from another session is never used for recovery' {
    ...
    $first = New-SessionId
    Invoke-Hook 'subagentStart' @{ sessionId = $first; cwd = $shared; agentName = 'autodev:autodev-architecture-review' } | Out-Null
    Invoke-Hook 'subagentStop' @{ sessionId = $first; cwd = $shared; agentName = 'autodev:autodev-architecture-review'; response = "ok`n`nAUTODEV-VERDICT: PASS" } | Out-Null

    $second = New-SessionId
    Invoke-Hook 'subagentStart' @{ sessionId = $second; cwd = $shared; agentName = 'autodev:autodev-architecture-review' } | Out-Null
...
Test-Case 'a stray mirror in the shared state directory does not mask a live session' {
    ...
    $stray = Join-Path (Join-Path $script:Root 'autodev\gates') 'gate-status.json'
...
Test-Case 'a workspace path needing JSON unescaping still resolves to the same answer' {
    ...
    Invoke-Hook 'subagentStart' @{ sessionId = $sid; cwd = $cwd; agentName = 'autodev:autodev-architecture-review' } | Out-Null
...
Test-Case 'two sessions in one directory keep independent attempt counters' {
    ...
    foreach ($i in 1..3) {
        foreach ($sid in @($a, $b)) {
            Invoke-Hook 'subagentStart' @{ sessionId = $sid; cwd = $shared; agentName = 'autodev:autodev-architecture-review' } | Out-Null
            Invoke-Hook 'subagentStop' @{ sessionId = $sid; cwd = $shared; agentName = 'autodev:autodev-architecture-review'; response = "finding`n`nAUTODEV-VERDICT: ISSUES" } | Out-Null
        }
    }
    foreach ($sid in @($a, $b)) {
    ...
    Invoke-Hook 'subagentStart' @{ sessionId = $a; cwd = $shared; agentName = 'autodev:autodev-architecture-review' } | Out-Null
    Invoke-Hook 'subagentStop' @{ sessionId = $a; cwd = $shared; agentName = 'autodev:autodev-architecture-review'; response = "finding`n`nAUTODEV-VERDICT: ISSUES" } | Out-Null

    $args2 = '{"agent_type":"autodev:autodev-architecture-review"}'
...
Test-Case 'a new session gets fresh state but preserves previous Markdown logs' {
    ...
    $first = New-SessionId
    foreach ($gate in @('architecture', 'security', 'privacy')) {
        Invoke-Hook 'subagentStart' @{ sessionId = $first; cwd = $shared; agentName = "autodev:autodev-$gate-review" } | Out-Null
        Invoke-Hook 'subagentStop' @{ sessionId = $first; cwd = $shared; agentName = "autodev:autodev-$gate-review"; response = "ok`n`nAUTODEV-VERDICT: PASS" } | Out-Null
    }
    ...
    Invoke-Hook 'subagentStart' @{ sessionId = $second; cwd = $shared; agentName = 'autodev:autodev-architecture-review' } | Out-Null
    Invoke-Hook 'subagentStop' @{ sessionId = $second; cwd = $shared; agentName = 'autodev:autodev-architecture-review'; response = "ok`n`nAUTODEV-VERDICT: PASS" } | Out-Null
...
Test-Case 'unrelated tool calls and sub-agents do not create a .autodev directory' {
    ...
    Invoke-Hook 'subagentStart' @{ sessionId = $sid; agentName = 'autodev:autodev-architecture-review' } | Out-Null
...
Test-Case 'every hook entry dispatches its own event name to the right script' {
    foreach ($prop in $script:Hooks.hooks.PSObject.Properties) {
        foreach ($entry in $prop.Value) {
            Assert-Match "autodev-router\.sh`" $($prop.Name)$" $entry.bash "bad bash wiring for $($prop.Name)"
            Assert-Match "autodev-router\.ps1`" $($prop.Name)$" $entry.powershell "bad powershell wiring for $($prop.Name)"
...
Test-Case 'every script referenced by hooks.json exists' {
    foreach ($rel in @('hooks/scripts/autodev-router.ps1', 'hooks/scripts/autodev-router.sh')) {
...
Test-Case 'the subagentStart matcher catches all three reviewer agents' {
    ...
    foreach ($gate in @('architecture', 'security', 'privacy')) {
        ...
        $name = "autodev:autodev-$gate-review"
...
function Invoke-TelemetryRound {
    ...
    $agentName = "autodev:autodev-$Gate-review"
    if (-not $SkipStart) {
...
Test-Case 'telemetry is off by default and writes nothing' {
    ...
    Invoke-HookWithEnv 'subagentStart' @{ sessionId = $session; agentName = 'autodev:autodev-architecture-review' } @{ AUTODEV_OTEL_DEBUG_FILE = $sink } | Out-Null
    $out = Invoke-HookWithEnv 'subagentStop' @{
        sessionId = $session
        agentName = 'autodev:autodev-architecture-review'
...
Test-Case 'the span carries the identifiers a backend needs to correlate' {
    ...
    $doc = $result.Spans[0]
    Assert-Equal 'autodev' (Get-SpanAttribute $doc 'autodev.plugin')
    ...
    Assert-Equal 'autodev:autodev-security-review' (Get-SpanAttribute $doc 'github.copilot.agent.name')
...
Test-Case 'a stop without its start still counts as an invocation' {
    ...
    Invoke-HookWithEnv 'subagentStop' @{
        sessionId = $session; agentName = 'autodev:autodev-architecture-review'; agentId = 'a1'
        response  = "x`n`nAUTODEV-VERDICT: ISSUES"
...
Test-Case 'a malformed traceparent falls back to a root span' {
    ...
    foreach ($tp in $bad) {
        ...
        Invoke-HookWithEnv 'subagentStop' @{
            sessionId   = $session; agentName = 'autodev:autodev-architecture-review'
