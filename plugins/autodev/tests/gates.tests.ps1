...
Test-Case 'every hook entry dispatches its own event name to the right script' {
    foreach ($prop in $script:Hooks.hooks.PSObject.Properties) {
        foreach ($entry in $prop.Value) {
            Assert-Match "autodev-(gates|stages)\.sh`" $($prop.Name)$" $entry.bash "bad bash wiring for $($prop.Name)"
            Assert-Match "autodev-(gates|stages)\.ps1`" $($prop.Name)$" $entry.powershell "bad powershell wiring for $($prop.Name)"
