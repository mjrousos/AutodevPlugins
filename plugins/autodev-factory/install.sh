...
if [ "$MODE" = "project" ]; then
  ...
  TARGET_DIR="$repo_root/.github/extensions/autodev-factory"
else
  TARGET_DIR="$copilot_home/extensions/autodev-factory"
fi
...
if [ "$MODE" = "uninstall" ]; then
  ...
  LEGACY_TARGET_DIR="${TARGET_DIR%/autodev-factory}/autodev"
  if [ -d "$LEGACY_TARGET_DIR" ] && [ -f "$LEGACY_TARGET_DIR/extension.mjs" ] && [ -f "$LEGACY_TARGET_DIR/prompts.generated.mjs" ]; then
    rm -rf "$LEGACY_TARGET_DIR"
    echo "removed legacy autodev runtime install -> $LEGACY_TARGET_DIR"
    removed=1
  fi
...
echo "run /extensions in Copilot CLI (or restart it), then invoke the 'autodev-factory' runtime."
