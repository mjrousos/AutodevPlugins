...
.DESCRIPTION
    ...
    After installing, run /extensions in the CLI (or restart it) and the 'autodev-factory' runtime becomes
...
if ($Scope -eq 'Project') {
    ...
    $targetDir = Join-Path ($repoRoot.Trim() -replace '/', '\') '.github\extensions\autodev-factory'
}
else {
    $copilotHome = if ($env:COPILOT_HOME) { $env:COPILOT_HOME } else { Join-Path $HOME '.copilot' }
    $targetDir = Join-Path $copilotHome 'extensions\autodev-factory'
}
...
if ($Uninstall) {
    ...
    $legacyTargetDir = Join-Path (Split-Path -Parent $targetDir) 'autodev'
    if ((Test-Path -LiteralPath $legacyTargetDir -PathType Container) -and
        (Test-Path -LiteralPath (Join-Path $legacyTargetDir 'extension.mjs') -PathType Leaf) -and
        (Test-Path -LiteralPath (Join-Path $legacyTargetDir 'prompts.generated.mjs') -PathType Leaf)) {
        Remove-Item -LiteralPath $legacyTargetDir -Recurse -Force
        Write-Host "removed legacy autodev runtime install -> $legacyTargetDir"
        $removed = $true
    }
...
Write-Host "run /extensions in Copilot CLI (or restart it), then invoke the 'autodev-factory' runtime."
