...
The reviewers and workers are the same ones the `autodev` plugin uses — literally the same instructions,
...
The `autodev` plugin's orchestrator agents are built the only way they could be: a markdown orchestrator that drives
...
therefore *cannot* delegate to `autodev:autodev-architecture-review`, however much it would
like to.

So `scripts/sync-autodev-prompts.sh` lifts each agent's body out of its canonical `.agent.md`,
along with the model its frontmatter declares, into `extensions/autodev-factory/prompts.generated.mjs`.
...
plugins/autodev-factory/install.sh

# Or just this repository:
plugins/autodev-factory/install.sh --project
```

The installer also removes the previous direct-install location (`extensions/autodev`) when it
contains the recognizable legacy factory files. It leaves an unrelated directory with that name
unchanged.
...
Or invoke it directly with `run_factory`, name `autodev-factory`:
...
```json
{ "name": "autodev-factory", "args": { "request": "…" }, "limits": { "maxAiCredits": 400 } }
```
...
The names deliberately differ from the `autodev` plugin's `gate-audit.md` and `feedback-log.md` so a factory
run and a plugin run in the same repository cannot overwrite each other's evidence.

### About hooks

The `autodev` plugin's `subagentStart`/`subagentStop` hooks **do not fire for factory subagents** — the gate
...
The prompt bundle is generated. Never edit `extensions/autodev-factory/prompts.generated.mjs` by hand:
...
It reads nine agents from the `autodev` plugin — the three plan reviewers, and tasking,
implementation, code review, code fix, code security review and code privacy review — plus
the plan document template from the `autodev-plan` orchestrator agent. The two orchestrator agents are
...
```bash
node --import ./plugins/autodev-factory/tests/register-stub.mjs --test plugins/autodev-factory/tests/factory.tests.mjs
```
